...
public class OptionsEntryValueInput<T> extends OptionsEntryValue<T> {
    ...
    public OptionsEntryValueInput(int width, String optionName, T value, Consumer<T> save, Predicate<String> validator) {
        super(optionName, save);

        this.value = value;
        this.textField = new WatchedTextfield(this, client.fontRenderer, 0, 0, width, 18);
        textField.setText(String.valueOf(value));
        textField.setValidator(validator);
    }
    ...
    protected void drawValue(MatrixStack matrixStack, int entryWidth, int entryHeight, int x, int y, int mouseX, int mouseY, boolean selected, float partialTicks) {
        textField.x = textField.getWidth() > 100 ? x : x + 135;
