...
public class MessageStripWorld {
    ...
    public static void handle(MessageStripWorld message, Supplier<Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            double chunkClearSizeX = message.x == 0 ? ConfigOptions.Stripping.blocksToStripX / 2F : message.x;
            double chunkClearSizeZ = message.z == 0 ? ConfigOptions.Stripping.blocksToStripZ / 2F : message.z;
