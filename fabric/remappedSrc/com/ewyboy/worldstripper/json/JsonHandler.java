...
public class JsonHandler {
    ...
    public static void setup() {
        createDirectory();
        if(!JSON_FILE.exists()) {
            ModLogger.getLogger().info("Creating World Stripper JSON file");
            writeJson(JSON_FILE);
        }
        ModLogger.getLogger().info("Reading World Stripper JSON file");
        readJson(JSON_FILE);
    }
    ...
    private static void createDirectory() {
        ...
        try {
            ModLogger.getLogger().info("Creating World Stripper directory");
            Files.createDirectory(path);
        } catch (FileAlreadyExistsException ignored) {
        } catch (IOException e) {
            ModLogger.getLogger().error("Failed to create world-stripper directory", e);
        }
    }
}
