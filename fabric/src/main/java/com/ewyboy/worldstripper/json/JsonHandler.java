...
public class JsonHandler {
    ...
    public static final File JSON_FILE = new File(FabricLoader.getInstance().getConfigDir() + "/worldstripper/profile-1.json");
    ...
    public static void setup() {
        createDirectory();
        if(!JSON_FILE.exists()) {
            ModLogger.getLogger().info("Creating World Stripper config JSON file");
            writeJson(JSON_FILE);
        }
        ModLogger.getLogger().info("Reading World Stripper config JSON file");
        readJson(JSON_FILE);
    }
    ...
    private static void createDirectory() {
        ...
        try {
            ModLogger.getLogger().info("Creating World Stripper config directory");
            Files.createDirectory(path);
        } catch (FileAlreadyExistsException ignored) {
        } catch (IOException e) {
            ModLogger.getLogger().error("Failed to create world-stripper config directory", e);
        }
    }
}
