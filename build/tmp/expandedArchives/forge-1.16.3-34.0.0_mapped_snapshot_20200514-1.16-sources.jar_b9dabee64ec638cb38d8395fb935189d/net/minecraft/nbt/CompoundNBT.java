...
public class CompoundNBT implements INBT {
   ...
   protected static ITextComponent func_197642_t(String p_197642_0_) {
      if (SIMPLE_VALUE.matcher(p_197642_0_).matches()) {
         return (new StringTextComponent(p_197642_0_)).func_240699_a_(SYNTAX_HIGHLIGHTING_KEY);
      } else {
         String s = StringNBT.quoteAndEscape(p_197642_0_);
         String s1 = s.substring(0, 1);
         ITextComponent itextcomponent = (new StringTextComponent(s.substring(1, s.length() - 1))).func_240699_a_(SYNTAX_HIGHLIGHTING_KEY);
         return (new StringTextComponent(s1)).func_230529_a_(itextcomponent).func_240702_b_(s1);
      }
   }
   ...
   public ITextComponent toFormattedComponent(String indentation, int indentDepth) {
      if (this.tagMap.isEmpty()) {
         ...
         for(Iterator<String> iterator = collection.iterator(); iterator.hasNext(); iformattabletextcomponent.func_230529_a_(iformattabletextcomponent1)) {
            String s = iterator.next();
            iformattabletextcomponent1 = (new StringTextComponent(Strings.repeat(indentation, indentDepth + 1))).func_230529_a_(func_197642_t(s)).func_240702_b_(String.valueOf(':')).func_240702_b_(" ").func_230529_a_(this.tagMap.get(s).toFormattedComponent(indentation, indentDepth + 1));
            if (iterator.hasNext()) {
         ...
         if (!indentation.isEmpty()) {
            iformattabletextcomponent.func_240702_b_("\n").func_240702_b_(Strings.repeat(indentation, indentDepth));
         }
