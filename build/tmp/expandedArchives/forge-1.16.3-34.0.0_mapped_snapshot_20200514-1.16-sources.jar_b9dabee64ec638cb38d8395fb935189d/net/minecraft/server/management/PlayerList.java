...
public abstract class PlayerList {
   ...
   public void initializeConnectionToPlayer(NetworkManager netManager, ServerPlayerEntity playerIn) {
      ...
      RegistryKey<World> registrykey = compoundnbt != null ? DimensionType.func_236025_a_(new Dynamic<>(NBTDynamicOps.INSTANCE, compoundnbt.get("Dimension"))).resultOrPartial(LOGGER::error).orElse(World.field_234918_g_) : World.field_234918_g_;
      ...
      if (compoundnbt != null && compoundnbt.contains("RootVehicle", 10)) {
         ...
         Entity entity1 = EntityType.func_220335_a(compoundnbt1.getCompound("Entity"), serverworld1, (p_217885_1_) -> {
   ...
   public CompoundNBT readPlayerDataFromFile(ServerPlayerEntity playerIn) {
      CompoundNBT compoundnbt = this.server.func_240793_aU_().func_230416_x_();
      ...
      if (playerIn.getName().getString().equals(this.server.getServerOwner()) && compoundnbt != null) {
         ...
         compoundnbt1 = this.playerDataManager.func_237336_b_(playerIn);
      }
   ...
   protected void writePlayerData(ServerPlayerEntity playerIn) {
      ...
      this.playerDataManager.func_237335_a_(playerIn);
   ...
   public ITextComponent canPlayerLogin(SocketAddress p_206258_1_, GameProfile p_206258_2_) {
      if (this.bannedPlayers.isBanned(p_206258_2_)) {
         ...
         if (profilebanentry.getBanEndDate() != null) {
            iformattabletextcomponent1.func_230529_a_(new TranslationTextComponent("multiplayer.disconnect.banned.expiration", DATE_FORMAT.format(profilebanentry.getBanEndDate())));
         }
         ...
         if (ipbanentry.getBanEndDate() != null) {
            iformattabletextcomponent.func_230529_a_(new TranslationTextComponent("multiplayer.disconnect.banned_ip.expiration", DATE_FORMAT.format(ipbanentry.getBanEndDate())));
         }
   ...
   public ServerPlayerEntity func_232644_a_(ServerPlayerEntity p_232644_1_, boolean p_232644_2_) {
      ...
      if (optional.isPresent()) {
         ...
         boolean flag1 = blockstate.isIn(Blocks.field_235400_nj_);
         Vector3d vector3d = optional.get();
         float f1;
         if (!blockstate.func_235714_a_(BlockTags.BEDS) && !flag1) {
            f1 = f;
         } else {
            Vector3d vector3d1 = Vector3d.func_237492_c_(blockpos).subtract(vector3d).normalize();
      ...
      serverplayerentity.connection.sendPacket(new SRespawnPacket(serverplayerentity.world.func_230315_m_(), serverplayerentity.world.func_234923_W_(), BiomeManager.func_235200_a_(serverplayerentity.getServerWorld().getSeed()), serverplayerentity.interactionManager.getGameType(), serverplayerentity.interactionManager.func_241815_c_(), serverplayerentity.getServerWorld().func_234925_Z_(), serverplayerentity.getServerWorld().func_241109_A_(), p_232644_2_));
      ...
      serverplayerentity.connection.sendPacket(new SWorldSpawnChangedPacket(serverworld1.func_241135_u_(), serverworld1.func_242107_v()));
      ...
      if (flag2) {
         serverplayerentity.connection.sendPacket(new SPlaySoundEffectPacket(SoundEvents.field_232818_ms_, SoundCategory.BLOCKS, (double)blockpos.getX(), (double)blockpos.getY(), (double)blockpos.getZ(), 1.0F, 1.0F));
      }
   ...
   public void func_232642_a_(IPacket<?> p_232642_1_, RegistryKey<World> p_232642_2_) {
      for(int i = 0; i < this.players.size(); ++i) {
         ServerPlayerEntity serverplayerentity = this.players.get(i);
         if (serverplayerentity.world.func_234923_W_() == p_232642_2_) {
   ...
   public void sendToAllNearExcept(@Nullable PlayerEntity except, double x, double y, double z, double radius, RegistryKey<World> dimension, IPacket<?> packetIn) {
      for(int i = 0; i < this.players.size(); ++i) {
         ServerPlayerEntity serverplayerentity = this.players.get(i);
         if (serverplayerentity != except && serverplayerentity.world.func_234923_W_() == dimension) {
   ...
   public void sendWorldInfo(ServerPlayerEntity playerIn, ServerWorld worldIn) {
      ...
      playerIn.connection.sendPacket(new SWorldSpawnChangedPacket(worldIn.func_241135_u_(), worldIn.func_242107_v()));
   ...
   public ServerStatisticsManager getPlayerStats(PlayerEntity playerIn) {
      ...
      if (serverstatisticsmanager == null) {
         File file1 = this.server.func_240776_a_(FolderName.field_237246_b_).toFile();
   ...
   public PlayerAdvancements getPlayerAdvancements(ServerPlayerEntity p_192054_1_) {
      ...
      if (playeradvancements == null) {
         File file1 = this.server.func_240776_a_(FolderName.field_237245_a_).toFile();
         File file2 = new File(file1, uuid + ".json");
   ...
   public void reloadResources() {
      for(PlayerAdvancements playeradvancements : this.advancements.values()) {
         playeradvancements.func_240918_a_(this.server.getAdvancementManager());
      }

      this.sendPacketToAllPlayers(new STagsListPacket(this.server.func_244266_aF()));
      net.minecraftforge.fml.network.NetworkHooks.syncCustomTagTypes(this.server.func_244266_aF());
