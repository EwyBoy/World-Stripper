...
public class DebugChunkGenerator extends ChunkGenerator {
   ...
   private static List<BlockState> ALL_VALID_STATES = StreamSupport.stream(Registry.BLOCK.spliterator(), false).flatMap((p_236067_0_) -> {
      return p_236067_0_.getStateContainer().getValidStates().stream();
   }).collect(Collectors.toList());
   private static int GRID_WIDTH = MathHelper.ceil(MathHelper.sqrt((float)ALL_VALID_STATES.size()));
   private static int GRID_HEIGHT = MathHelper.ceil((float)ALL_VALID_STATES.size() / (float)GRID_WIDTH);
   ...
   public static void initValidStates() {
      ALL_VALID_STATES = StreamSupport.stream(Registry.BLOCK.spliterator(), false).flatMap(block -> block.getStateContainer().getValidStates().stream()).collect(Collectors.toList());
