...
public class ShootTargetTask<E extends MobEntity & ICrossbowUser, T extends LivingEntity> extends Task<E> {
   ...
   public ShootTargetTask() {
      super(ImmutableMap.of(MemoryModuleType.LOOK_TARGET, MemoryModuleStatus.REGISTERED, MemoryModuleType.field_234103_o_, MemoryModuleStatus.VALUE_PRESENT), 1200);
   }

   protected boolean shouldExecute(ServerWorld worldIn, E owner) {
      LivingEntity livingentity = func_233887_a_(owner);
      return owner.func_233631_a_(Items.CROSSBOW) && BrainUtil.func_233876_c_(owner, livingentity) && BrainUtil.func_233869_a_(owner, livingentity, 0);
   }

   protected boolean shouldContinueExecuting(ServerWorld worldIn, E entityIn, long gameTimeIn) {
      return entityIn.getBrain().hasMemory(MemoryModuleType.field_234103_o_) && this.shouldExecute(worldIn, entityIn);
   }
   ...
   protected void resetTask(ServerWorld worldIn, E entityIn, long gameTimeIn) {
      ...
      if (entityIn.func_233631_a_(Items.CROSSBOW)) {
   ...
   private static LivingEntity func_233887_a_(LivingEntity p_233887_0_) {
      return p_233887_0_.getBrain().getMemory(MemoryModuleType.field_234103_o_).get();
   }
