...
public class WalkToHouseTask extends Task<LivingEntity> {
   ...
   protected boolean shouldExecute(ServerWorld worldIn, LivingEntity owner) {
      if (worldIn.getGameTime() - this.field_220525_b < 20L) {
         ...
         Optional<BlockPos> optional = pointofinterestmanager.func_234148_d_(PointOfInterestType.HOME.getPredicate(), owner.func_233580_cy_(), 48, PointOfInterestManager.Status.ANY);
         return optional.isPresent() && !(optional.get().distanceSq(creatureentity.func_233580_cy_()) <= 4.0D);
      }
   }
   ...
   protected void startExecuting(ServerWorld worldIn, LivingEntity entityIn, long gameTimeIn) {
      ...
      Stream<BlockPos> stream = pointofinterestmanager.findAll(PointOfInterestType.HOME.getPredicate(), predicate, entityIn.func_233580_cy_(), 48, PointOfInterestManager.Status.ANY);
