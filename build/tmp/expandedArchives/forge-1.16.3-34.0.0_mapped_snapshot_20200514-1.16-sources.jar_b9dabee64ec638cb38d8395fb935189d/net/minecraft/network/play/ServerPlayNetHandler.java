...
public class ServerPlayNetHandler implements IServerPlayNetHandler {
   ...
   private boolean func_241162_a_(Entity p_241162_1_) {
      return BlockPos.func_239581_a_(p_241162_1_.getBoundingBox().grow(0.0625D).expand(0.0D, -0.55D, 0.0D)).allMatch(b -> p_241162_1_.world.getBlockState(b).isAir(p_241162_1_.world, b));
   }
   ...
   public void processUpdateCommandBlock(CUpdateCommandBlockPacket packetIn) {
      ...
      if (!this.server.isCommandBlockEnabled()) {
         this.player.sendMessage(new TranslationTextComponent("advMode.notEnabled"), Util.field_240973_b_);
      } else if (!this.player.canUseCommandBlock()) {
         this.player.sendMessage(new TranslationTextComponent("advMode.notAllowed"), Util.field_240973_b_);
      } else {
         ...
         if (commandblocklogic != null) {
            ...
            if (!StringUtils.isNullOrEmpty(s)) {
               this.player.sendMessage(new TranslationTextComponent("advMode.setCommand.success", s), Util.field_240973_b_);
            }
         }
   ...
   public void processUpdateCommandMinecart(CUpdateMinecartCommandBlockPacket packetIn) {
      ...
      if (!this.server.isCommandBlockEnabled()) {
         ...
         if (commandblocklogic != null) {
            ...
            commandblocklogic.updateCommand();
            this.player.sendMessage(new TranslationTextComponent("advMode.setCommand.success", packetIn.getCommand()), Util.field_240973_b_);
         }
            ...
            if (this.targetPos != null) {
               ...
               if (this.player.isPassenger()) {
                  ...
                  if (this.player.isSleeping()) {
                     ...
                     if (this.player.func_233570_aj_() && !packetIn.isOnGround() && flag) {
                        this.player.jump();
                     }
                     ...
                     if (this.player.noClip || this.player.isSleeping() || (!flag1 || !serverworld.hasNoCollisions(this.player, axisalignedbb)) && !this.func_241163_a_(serverworld, axisalignedbb)) {
                        ...
                        this.player.func_230245_c_(packetIn.isOnGround());
                        if (flag) {
      ...
      if (blockpos.getY() < this.server.getBuildLimit()) {
         ...
         if (this.targetPos == null && this.player.getDistanceSq((double)blockpos.getX() + 0.5D, (double)blockpos.getY() + 0.5D, (double)blockpos.getZ() + 0.5D) < dist && serverworld.isBlockModifiable(this.player, blockpos)) {
            ...
            if (direction == Direction.UP && !actionresulttype.isSuccessOrConsume() && blockpos.getY() >= this.server.getBuildLimit() - 1 && func_241166_a_(this.player, itemstack)) {
               ITextComponent itextcomponent = (new TranslationTextComponent("build.tooHigh", this.server.getBuildLimit())).func_240699_a_(TextFormatting.RED);
               this.player.connection.sendPacket(new SChatPacket(itextcomponent, ChatType.GAME_INFO, Util.field_240973_b_));
            } else if (actionresulttype.isSuccess()) {
               this.player.swing(hand, true);
            }
         }
      } else {
         ITextComponent itextcomponent1 = (new TranslationTextComponent("build.tooHigh", this.server.getBuildLimit())).func_240699_a_(TextFormatting.RED);
         this.player.connection.sendPacket(new SChatPacket(itextcomponent1, ChatType.GAME_INFO, Util.field_240973_b_));
      }
   ...
   public void onDisconnect(ITextComponent reason) {
      ...
      this.server.getPlayerList().func_232641_a_((new TranslationTextComponent("multiplayer.player.left", this.player.getDisplayName())).func_240699_a_(TextFormatting.YELLOW), ChatType.SYSTEM, Util.field_240973_b_);
   ...
   public void processChatMessage(CChatMessagePacket packetIn) {
      ...
      if (this.player.getChatVisibility() == ChatVisibility.HIDDEN) {
         this.sendPacket(new SChatPacket((new TranslationTextComponent("chat.cannotSend")).func_240699_a_(TextFormatting.RED), ChatType.SYSTEM, Util.field_240973_b_));
      } else {
         this.player.markPlayerActive();
      ...
      if (entity != null) {
         ...
         if (this.player.getDistanceSq(entity) < 36.0D) {
            ...
            if (packetIn.getAction() == CUseEntityPacket.Action.INTERACT) {
               ...
               if (net.minecraftforge.common.ForgeHooks.onInteractEntityAt(player, entity, packetIn.getHitVec(), hand) != null) return;
            ...
            if (optional.isPresent() && optional.get().isSuccessOrConsume()) {
               CriteriaTriggers.field_241276_P_.func_241476_a_(this.player, itemstack, entity);
   ...
   public void processClientStatus(CClientStatusPacket packetIn) {
      ...
      case PERFORM_RESPAWN:
         if (this.player.queuedEndExit) {
            ...
            CriteriaTriggers.CHANGED_DIMENSION.func_233551_a_(this.player, World.field_234920_i_, World.field_234918_g_);
         } else {
