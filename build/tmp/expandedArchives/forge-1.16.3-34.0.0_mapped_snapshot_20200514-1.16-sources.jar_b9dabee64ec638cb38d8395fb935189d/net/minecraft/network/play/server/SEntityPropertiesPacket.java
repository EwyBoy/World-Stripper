...
public class SEntityPropertiesPacket implements IPacket<IClientPlayNetHandler> {
   ...
   public SEntityPropertiesPacket(int entityIdIn, Collection<ModifiableAttributeInstance> instances) {
      ...
      for(ModifiableAttributeInstance modifiableattributeinstance : instances) {
         this.snapshots.add(new SEntityPropertiesPacket.Snapshot(modifiableattributeinstance.getAttribute(), modifiableattributeinstance.getBaseValue(), modifiableattributeinstance.func_225505_c_()));
      }
   ...
   public void readPacketData(PacketBuffer buf) throws IOException {
      ...
      for(int j = 0; j < i; ++j) {
         ...
         Attribute attribute = Registry.field_239692_aP_.getOrDefault(resourcelocation);
   ...
   public void writePacketData(PacketBuffer buf) throws IOException {
      ...
      for(SEntityPropertiesPacket.Snapshot sentitypropertiespacket$snapshot : this.snapshots) {
         buf.writeResourceLocation(Registry.field_239692_aP_.getKey(sentitypropertiespacket$snapshot.func_240834_a_()));
