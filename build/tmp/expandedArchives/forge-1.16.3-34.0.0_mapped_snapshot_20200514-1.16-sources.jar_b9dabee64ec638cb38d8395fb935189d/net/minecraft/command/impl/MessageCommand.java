...
public class MessageCommand {
   ...
   private static int sendPrivateMessage(CommandSource source, Collection<ServerPlayerEntity> recipients, ITextComponent message) {
      UUID uuid = source.getEntity() == null ? Util.field_240973_b_ : source.getEntity().getUniqueID();
      Entity entity = source.getEntity();
      Consumer<ITextComponent> consumer;
      if (entity instanceof ServerPlayerEntity) {
         ServerPlayerEntity serverplayerentity = (ServerPlayerEntity)entity;
         consumer = (p_244374_2_) -> {
            serverplayerentity.sendMessage((new TranslationTextComponent("commands.message.display.outgoing", p_244374_2_, message)).func_240701_a_(new TextFormatting[]{TextFormatting.GRAY, TextFormatting.ITALIC}), serverplayerentity.getUniqueID());
         };
      } else {
         consumer = (p_244375_2_) -> {
            source.sendFeedback((new TranslationTextComponent("commands.message.display.outgoing", p_244375_2_, message)).func_240701_a_(new TextFormatting[]{TextFormatting.GRAY, TextFormatting.ITALIC}), false);
         };
      }

      for(ServerPlayerEntity serverplayerentity1 : recipients) {
         consumer.accept(serverplayerentity1.getDisplayName());
         serverplayerentity1.sendMessage((new TranslationTextComponent("commands.message.display.incoming", source.getDisplayName(), message)).func_240701_a_(new TextFormatting[]{TextFormatting.GRAY, TextFormatting.ITALIC}), uuid);
      }
