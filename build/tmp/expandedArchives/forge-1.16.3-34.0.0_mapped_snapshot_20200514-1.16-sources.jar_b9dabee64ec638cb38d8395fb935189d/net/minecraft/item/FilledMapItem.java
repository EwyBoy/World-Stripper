...
public class FilledMapItem extends AbstractMapItem {
   ...
   public static ItemStack setupNewMap(World worldIn, int worldX, int worldZ, byte scale, boolean trackingPosition, boolean unlimitedTracking) {
      ...
      createMapData(itemstack, worldIn, worldX, worldZ, scale, trackingPosition, unlimitedTracking, worldIn.func_234923_W_());
      return itemstack;
   }
   ...
   protected MapData getCustomMapData(ItemStack stack, World worldIn) {
      ...
      if (mapdata == null && worldIn instanceof ServerWorld) {
         mapdata = createMapData(stack, worldIn, worldIn.getWorldInfo().getSpawnX(), worldIn.getWorldInfo().getSpawnZ(), 3, false, false, worldIn.func_234923_W_());
      }
   ...
   private static MapData createMapData(ItemStack stack, World worldIn, int x, int z, int scale, boolean trackingPosition, boolean unlimitedTracking, RegistryKey<World> dimensionTypeIn) {
      ...
      mapdata.func_237241_a_(x, z, scale, trackingPosition, unlimitedTracking, dimensionTypeIn);
   ...
   public void updateMapData(World worldIn, Entity viewer, MapData data) {
      if (worldIn.func_234923_W_() == data.dimension && viewer instanceof PlayerEntity) {
         int i = 1 << data.scale;
         int j = data.xCenter;
         int k = data.zCenter;
   ...
   public static void func_226642_a_(ServerWorld p_226642_0_, ItemStack p_226642_1_) {
      ...
      if (mapdata != null) {
         if (p_226642_0_.func_234923_W_() == mapdata.dimension) {
   ...
   public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
      ...
      if (mapdata != null && mapdata.locked) {
         tooltip.add((new TranslationTextComponent("filled_map.locked", getMapId(stack))).func_240699_a_(TextFormatting.GRAY));
      }

      if (flagIn.isAdvanced()) {
         if (mapdata != null) {
            tooltip.add((new TranslationTextComponent("filled_map.id", getMapId(stack))).func_240699_a_(TextFormatting.GRAY));
            tooltip.add((new TranslationTextComponent("filled_map.scale", 1 << mapdata.scale)).func_240699_a_(TextFormatting.GRAY));
            tooltip.add((new TranslationTextComponent("filled_map.level", mapdata.scale, 4)).func_240699_a_(TextFormatting.GRAY));
         } else {
            tooltip.add((new TranslationTextComponent("filled_map.unknown")).func_240699_a_(TextFormatting.GRAY));
         }
      }
   ...
   public ActionResultType onItemUse(ItemUseContext context) {
      ...
      if (blockstate.func_235714_a_(BlockTags.BANNERS)) {
