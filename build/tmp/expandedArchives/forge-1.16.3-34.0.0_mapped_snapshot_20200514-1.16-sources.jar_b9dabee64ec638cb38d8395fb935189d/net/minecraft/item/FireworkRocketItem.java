   ...
   public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
      ...
      if (compoundnbt != null) {
         if (compoundnbt.contains("Flight", 99)) {
            tooltip.add((new TranslationTextComponent("item.minecraft.firework_rocket.flight")).func_240702_b_(" ").func_240702_b_(String.valueOf((int)compoundnbt.getByte("Flight"))).func_240699_a_(TextFormatting.GRAY));
         }
            ...
            for(int i = 0; i < listnbt.size(); ++i) {
               ...
               if (!list.isEmpty()) {
                  for(int j = 1; j < list.size(); ++j) {
                     list.set(j, (new StringTextComponent("  ")).func_230529_a_(list.get(j)).func_240699_a_(TextFormatting.GRAY));
                  }
