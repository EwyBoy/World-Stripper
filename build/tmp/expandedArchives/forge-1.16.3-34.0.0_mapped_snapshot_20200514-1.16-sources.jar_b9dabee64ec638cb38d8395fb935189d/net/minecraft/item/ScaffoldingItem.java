         ...
         while(i < 7) {
            if (!world.isRemote && !World.isValid(blockpos$mutable)) {
               ...
               if (playerentity instanceof ServerPlayerEntity && blockpos$mutable.getY() >= j) {
                  SChatPacket schatpacket = new SChatPacket((new TranslationTextComponent("build.tooHigh", j)).func_240699_a_(TextFormatting.RED), ChatType.GAME_INFO, Util.field_240973_b_);
